...
public interface IPeriodicTaskService
{
    ...
    Task ReorderGroupsAsync(IReadOnlyList<ReorderPeriodicGroupRequest> req, CancellationToken ct);
    ...
    Task DeleteTaskAsync(Guid id, ConfirmDeletePeriodicTaskRequest req, CancellationToken ct);
    Task ReorderTasksAsync(IReadOnlyList<ReorderPeriodicTaskRequest> req, CancellationToken ct);
