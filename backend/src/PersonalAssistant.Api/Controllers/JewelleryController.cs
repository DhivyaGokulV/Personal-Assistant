...
[ApiController]
[Authorize]
[Route("api/asset-tracker/jewellery")]
public class JewelleryController : ControllerBase
{
    ...
    public JewelleryController(IJewelleryService service, IReportExportService exporter)
    {
        _service = service;
        _exporter = exporter;
    }
    ...
    [HttpGet]
    public async Task<ActionResult<AssetTrackerPage<JewelleryDto>>> List(
    ...
    [HttpPost]
    public async Task<ActionResult<JewelleryDto>> Create(SaveJewelleryRequest request, CancellationToken ct) =>
        await Execute(() => _service.CreateAsync(request, ct));

    [HttpPatch("{id:guid}")]
    public async Task<ActionResult<JewelleryDto>> Update(Guid id, SaveJewelleryRequest request, CancellationToken ct) =>
        await Execute(() => _service.UpdateAsync(id, request, ct));

    [HttpPost("{id:guid}/sell")]
    public async Task<ActionResult<JewelleryDto>> Sell(Guid id, SellPossessionRequest request, CancellationToken ct) =>
    ...
    public async Task<IActionResult> Export(PossessionExportRequest request, CancellationToken ct)
    {
        try
        {
            ...
            var table = new ReportTable(
                $"Jewellery-{request.From:yyyyMMdd}-{request.To:yyyyMMdd}",
                new[] { "Name", "Description", "Buying date", "Buying price", "Quantity grams", "Status", "Selling date", "Selling price", "Selling note" },
                rows.Select(x => (IReadOnlyList<string?>)new[]
                {
                    x.Name, x.Description, x.BuyingDate.ToString("yyyy-MM-dd"), x.BuyingPrice.ToString("0.####"),
                    x.QuantityInGrams.ToString("0.####"), x.Status.ToString(), x.SellingDate?.ToString("yyyy-MM-dd"),
                    x.SellingPrice?.ToString("0.####"), x.SellingNote
                }).ToList());
